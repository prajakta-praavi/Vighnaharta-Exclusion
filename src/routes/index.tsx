...
import heroImg from "@/assets/extrusion-machine.jpeg";
import aboutImg from "@/assets/about.jpg";
import { aboutCopy, aboutValues, company, dieHeadModels, machineModels, motto, productHighlights, productLineup2026, screwBarrelSet, supportPromise } from "@/content/extrusion";
...
function Hero() {
  return (
    <section className="relative overflow-hidden text-white">
      <img src={heroImg} alt="Extrusion machine line" className="absolute inset-0 h-full w-full object-cover" width={1400} height={900} />
      <div className="absolute inset-0 bg-gradient-to-br from-[oklch(0.18_0.08_258/0.94)] via-[oklch(0.24_0.08_245/0.82)] to-[oklch(0.28_0.08_120/0.68)]" />
      <div className="container-x relative py-24 md:py-36 lg:py-40">
        <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-medium backdrop-blur">
          <span className="h-2 w-2 rounded-full bg-safety animate-pulse" />
          Established 2023 - Extrusion project and spare support
        </div>
        <h1 className="mt-6 max-w-4xl text-5xl md:text-7xl font-bold tracking-tight">
          {company.name}
        </h1>
        <p className="mt-6 max-w-2xl text-lg md:text-xl text-white/85">
          Quality extrusion machine spares, 3-layer pipe automatic machine setup, die heads and professional process support as per customer requirement.
        </p>
        <div className="mt-9 flex flex-wrap gap-3">
          <Link to="/contact" className="inline-flex items-center gap-2 rounded-md bg-safety px-6 py-3.5 font-semibold text-safety-foreground shadow-lg hover:brightness-110 transition">
            Request Quote <ArrowRight className="h-4 w-4" />
          </Link>
          <Link to="/products" className="inline-flex items-center gap-2 rounded-md border border-white/30 bg-white/10 px-6 py-3.5 font-semibold text-white backdrop-blur hover:bg-white/15 transition">
            View Models
          </Link>
        </div>
      </div>
  ...
  return (
    <section className="bg-brand-deep text-white">
      <div className="container-x grid grid-cols-2 md:grid-cols-4 gap-px bg-white/10">
        {stats.map((s) => (
          <div key={s.l} className="bg-brand-deep p-6 md:p-8 text-center">
            <div className="font-display text-3xl md:text-4xl font-bold text-safety">{s.v}</div>
            <div className="mt-1 text-xs md:text-sm uppercase tracking-wider opacity-80">{s.l}</div>
          </div>
        ))}
      </div>
    </section>
...
function SectionHeader({ eyebrow, title, sub }: { eyebrow: string; title: string; sub?: string }) {
  return (
    <div className="max-w-2xl">
      ...
      <h2 className="mt-3 text-3xl md:text-4xl font-bold text-brand">{title}</h2>
    ...
    <section className="py-20 md:py-28">
      <div className="container-x grid lg:grid-cols-2 gap-12 items-center">
        <div className="relative">
          <img src={aboutImg} alt="Industrial extrusion workshop" loading="lazy" width={1200} height={800} className="rounded-xl shadow-2xl" />
      ...
      <div className="container-x">
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {productLineup2026.map((item) => (
            <div key={item.title} className="bg-card border border-border rounded-xl p-6 hover:border-safety hover:shadow-xl transition">
        ...
        <div className="mt-10 grid lg:grid-cols-[1fr_1.2fr] gap-6">
          <div className="rounded-xl bg-brand p-7 text-brand-foreground">
            ...
            <blockquote className="mt-4 text-2xl font-bold leading-snug">"{motto}"</blockquote>
  ...
  return (
    <section className="relative py-20 md:py-24 bg-brand text-brand-foreground overflow-hidden">
      <div className="container-x relative grid lg:grid-cols-[2fr_1fr] gap-10 items-center">
        <div>
          <h2 className="text-3xl md:text-5xl font-bold">Need extrusion machine spares or die-head support?</h2>
          <p className="mt-4 max-w-2xl opacity-85 text-lg">Talk to our team about your machine model, pipe range, output target and process problem.</p>
        </div>
        <div className="flex flex-wrap gap-3 lg:justify-end">
          <Link to="/contact" className="inline-flex items-center gap-2 rounded-md bg-safety px-6 py-3.5 font-semibold text-safety-foreground hover:brightness-110 transition">Send Inquiry <ArrowRight className="h-4 w-4" /></Link>
        </div>
      </div>
    </section>
  );
