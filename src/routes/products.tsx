...
import { company, dieHeadModels, machineModels, motto, productHighlights, screwBarrelSet, supportPromise } from "@/content/extrusion";
...
export default function ProductsPage() {
  return (
    <SiteLayout>
      <PageHero eyebrow="Products" title="Products category-wise from the uploaded documents." subtitle="Each category now keeps its own document details, model data and related product images together." />
      <Breadcrumb items={[{ to: "/", label: "Home" }, { label: "Products" }]} />

      <section className="py-12 border-b border-border bg-secondary/50">
        <div className="container-x flex flex-wrap gap-2">
          {productCategories.map((category) => (
            <a key={category.id} href={`#${category.id}`} className="rounded-md border border-border bg-card px-3 py-2 text-sm font-semibold text-brand hover:border-safety hover:text-safety transition">
              {category.title}
            </a>
          ))}
        </div>
      ...
      <section className="py-20 bg-brand text-brand-foreground">
        <div className="container-x grid lg:grid-cols-[1.2fr_0.8fr] gap-8 items-center">
          <div>
            ...
            <blockquote className="mt-4 text-3xl md:text-4xl font-bold leading-tight">"{motto}"</blockquote>
            <p className="mt-4 max-w-2xl opacity-85">{supportPromise}</p>
          </div>
          <div className="flex flex-wrap gap-3 lg:justify-end">
            <Link to="/contact" className="inline-flex items-center gap-2 rounded-md bg-safety px-6 py-3.5 font-semibold text-safety-foreground hover:brightness-110 transition">Request quote <ArrowRight className="h-4 w-4" /></Link>
            <button className="inline-flex items-center gap-2 rounded-md border border-white/25 px-6 py-3.5 font-semibold hover:bg-white/10 transition">
...
function CategoryImages({ title, images }: { title: string; images: string[] }) {
  if (!images.length) {
    return (
      <div className="min-h-[260px] bg-brand text-brand-foreground p-8 flex items-center">
        <div>
